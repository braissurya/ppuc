...
public class HakBiayaController extends ParentController {
	...
	public String list(@RequestParam(value = "page", required = false) Integer page, @RequestParam(value = "size", required = false) Integer size, @RequestParam(value = "search", required = false) String search, @RequestParam(value = "sortFieldName", required = false) String sortFieldName,
		...
		if (Utils.isEmpty(gl))
			...
			gl = gl.substring(gl.lastIndexOf(".") + 1);
		if (Utils.isEmpty(pr))
			pr = null;
		else 
			pr = pr.substring(pr.lastIndexOf(".") + 1);
		if (Utils.isEmpty(kt)) 
			kt = null;
		else 
			kt = kt.substring(kt.lastIndexOf(".") + 1);
		if (Utils.isEmpty(lk)) 
			lk = null;
		else 
			lk = lk.substring(lk.lastIndexOf(".") + 1);
		if (Utils.isEmpty(gb))
			gb = null;
		else 
			gb = gb.substring(gb.lastIndexOf(".") + 1);
		if (Utils.isEmpty(kb)) 
			kb = null;
		else 
			kb = kb.substring(kb.lastIndexOf(".") + 1);
